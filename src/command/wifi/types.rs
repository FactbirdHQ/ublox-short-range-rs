...
pub enum WifiStationConfig {
    ...
    /// WEP Keys - <param_val1>...<param_val4> are the WEP encryption keys. A WEP
    /// key is either 5 bytes (while using WEP 64), or 13 bytes (while using WEP 128) and if not
    /// used, it can be empty. The keys must be in HEX data format. For example, 0102030405
    ///  while using WEP 64, or 0102030405060708090A0B0C0D while using WEP 128.
    ///
    /// "WEP Shared Key Authentication" is not supported; only "WEP Open Key
    /// Authentication " is supported.
    #[at_arg(value = 6)]
    WEPKeys(String<consts::U64>, Option<String<consts::U64>>, Option<String<consts::U64>>, 
        Option<String<consts::U64>>, Option<String<consts::U64>>),
    ///  Active Key - <param_val1> is the WEP active TX key (factory default 0 means that
    /// Open authentication with WEP encryption is disabled). Range 1-4.
    #[at_arg(value = 7)]
    ActiveKey(u8),
    ...
    /// ASCII characters as a string) for WPA/WPA2 PSK.
    #[at_arg(value = 8)]
    WPA_PSKOrPassphrase(String<consts::U64>),
    /// Password - <param_val1> is the password for LEAP and PEAP; string with a
    /// maximum length of 31.
    #[at_arg(value = 9)]
    EAPPassword(String<consts::U64>),
    /// User name - <param_val1> is the public user name for LEAP and PEAP; string with
    /// a maximum length of 31.
    #[at_arg(value = 10)]
...
pub enum WifiStationAction {
    /// It clears the specified profile and resets all the parameters to their factory
    /// defaults.
    Reset = 0,
    ...
    Load = 2,
    /// Validates the configuration, calculates the PSK for WPA and WPA2 (if not
    /// already calculated), and activates the specified profile from run-time memory. It will
    /// try to connect, if not connected.
    Activate = 3,
    /// It deactivates the specified profile. Disconnects the profile if connected,
    /// and may reconnect to other active network.
    Deactivate = 4,
...
pub enum WifiConfig{
    ...
    /// • On: Drop the network when the Wi-Fi link is lost; data may be lost with this option.
    /// Supported software versions 5.0.0 onwards
    #[at_arg(value = 10)]
    DropNetworkOnLinkLoss(OnOff),
    /// Force world mode
    /// • Off: Use all channels in the channel list; See +UWCL for more information. The
    ///      channel list will be filtered by 802.11d.
    /// • On (default): Lock device to world mode. The channel list (+UWCL) is filtered and only
    ...
    /// • On: Filter scan results; the module will try to only send one scan response for each
    ...
    /// • On: Enable block acknowledgement
...
pub enum AccessPointConfig{
    ...
    #[at_arg(value = 19)]
    WhiteList((String<consts::U64>,String<consts::U64>,String<consts::U64>)),
    ...
    #[at_arg(value = 20)]
    BlackList((String<consts::U64>,String<consts::U64>,String<consts::U64>)),
    /// IPv4 Mode - <param_val1> to set the way to retrieve an IP address
    /// • 1:(default) Static
    #[at_arg(value = 100)]
    IPv4Mode(IPv4Mode),
