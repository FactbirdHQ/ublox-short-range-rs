...
fn main() {
  ...
  port
    .set_timeout(Duration::from_millis(2))
  ...
  let mut ublox = ublox_short_range::UbloxClient::new(wifi_client);

  let at_parser_arc = Arc::new(Mutex::new(parser));

  let at_parser = at_parser_arc.clone();
  let serial_irq = thread::Builder::new().name("serial_irq".to_string()).spawn(move || loop {
    thread::sleep(Duration::from_millis(1));
    if let Ok(mut at) = at_parser.lock() { at.handle_irq() }
  }).unwrap();

  let at_parser = at_parser_arc.clone();
  let serial_loop = thread::Builder::new().name("serial_loop".to_string()).spawn(move || loop {
    thread::sleep(Duration::from_millis(100));
    if let Ok(mut at) = at_parser.lock() { at.spin() }
  }).unwrap();

  let main_loop = thread::Builder::new().name("main_loop".to_string()).spawn(move || {

    info!("Initializing AT to a known state..");
    ublox.init().expect("Cannot restore AT to a known good state");
    info!("AT is good!");

    // info!("Sending!");

    // let res = wifi_client.send(Command::ExecSTAAction {
    //     configuration_id: 0,
