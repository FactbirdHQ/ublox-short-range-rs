                ...
                match resp[6].into() {
                    ...
                    ConnectType::IPv6 => {
                        ...
                        let event = IPv6ConnectEvent{
                            channel_id: resp[5],
                            protocol: resp[7].into(),
                            remote_ip: Ipv6Addr::from([resp[8], resp[9], resp[10], resp[11], resp[12], resp[13], resp[14], resp[15], 
                                resp[16], resp[17], resp[18], resp[19], resp[20], resp[21], resp[22], resp[23]]),
                            remote_port: ((resp[24] as u16) << 8) | resp[25] as u16,
                            local_ip: Ipv6Addr::from([resp[26], resp[27], resp[28], resp[29], resp[30], resp[31], resp[32], resp[33], 
                                                      resp[34], resp[35], resp[36], resp[37], resp[38], resp[39], resp[40], resp[41]]),
...
mod test {
    ...
    fn parse_ipv4_connect_event(){
        // AT-urc: +UUDPD:3
        let resp = &[ 
            0xAA, 0x00, 0x11, 0x00, 0x11, 0x05, 0x02, 0x00, 0xC0, 0xA8, 0x00, 0x02, 0x13, 0x88, 0xC0, 0xA8, 0x00, 0x01, 0x0F, 0xA0, 0x55,
        ];
    ...
    fn parse_ipv6_connect_event(){
        // AT-event: +UUDPD:3
        let resp = &[ 
            0xAA, 0x00, 0x29, 0x00, 0x11, 0x05, 0x03, 0x00, 0xFE, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
            0x00, 0x02, 0x13, 0x88, 0xFE, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x0F, 0xA0, 0x55,
        ];
